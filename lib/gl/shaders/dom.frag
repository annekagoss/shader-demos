...
const float GRADIENT_STEPS = 15.0;
...
void main() {
	...
	vec3 darkHSL = vec3(1.0 - HSLColor.x, HSLColor.y, min(HSLColor.z, .25));
	vec3 lightHSL = vec3(HSLColor.x, HSLColor.y, max(HSLColor.z, .75));
	vec4 colorGradient = vec4(mix(hslTorgb(darkHSL), hslTorgb(lightHSL), floor(gradientSt.y * GRADIENT_STEPS)*(1.0/GRADIENT_STEPS)), uColor.a);
	
	vec4 color;
	if (backgroundLuminance > 0.5) {
		color = mix(colorGradient, vec4(image.rgb, 1.0), image.a);
	} else {
