...
float softShadow(vec4 position, sampler2D depthMap) {
  float sum = 0.0;
  sum += hardShadow(position, -5., depthMap) * 0.02;
  sum += hardShadow(position, -4., depthMap) * 0.05;
  sum += hardShadow(position, -3., depthMap) * 0.09;
  sum += hardShadow(position, -2., depthMap) * 0.12;
  sum += hardShadow(position, -1., depthMap) * 0.15;
