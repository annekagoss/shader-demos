vec4 toonShading(vec3 eye, vec4 normalDirection, vec4 vertexPosition,
                 vec3 lightPosition, vec3 baseColor) {
  ...
  float toon = 0.5 * smoothstep(.5, .51, lambert) + 0.5 +
  ...
  bool lightSourceOnRightSide =
      pow(max(0.0, dot(reflect(-lightDirection, normalDirection.xyz),
                       viewDirection)),
          10.0) > 0.8;
